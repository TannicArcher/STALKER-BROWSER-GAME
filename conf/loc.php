...
///////////////////////////////////////Все данные о локации
  ...
  <p><?php
	    ...
	    if ($row_loc['bp_gruppa'] == 'mon') {?><img src="img/ico/monolit.png" width="12" height="12" alt="n"/><?php }?> 
    <a href="attack.php?attack=bp">Стрелять по 
    <?php
  if ($row_loc['bp_gruppa'] == 'dolg') { echo 'долговцам'; }
  if ($row_loc['bp_gruppa'] == 'naemniki') { echo 'одиночкам'; }
  if ($row_loc['bp_gruppa'] == 'svoboda') { echo 'свободовцам'; }
  if ($row_loc['bp_gruppa'] == 'mon') { echo 'монолитовцам'; }?>
  </a>
  <?php
  ...
  if (isset($attack)) {
  $gruppa = $row['gruppa']; 
  $query_att = "Select id, nick, hp, gruppa, max_hp, last_active, radiation, regen, antirad_time from users where id = '$attack' and location = '$location' and last_active > NOW() - (1000) and hp != 0 and gruppa != '$gruppa' and ban != 1 limit 1";
...
/////////////////////////////////////////
	  if ($row['speed_p'] != 0) {				
		?>
	    <p><img src="img/ico/pistol.png" alt="p" width="12" height="12"/> <a href="attack.php?rand=0&weapon=pistol&attack=<?php echo"$attack"; ?>">Стрелять с пистолета</a> <?php
		if ($gruppa_us == 'svoboda') {?><img src="img/ico/svobodaon.png" width="12" height="12" alt="s"/><?php }
	    if ($gruppa_us == 'dolg') {?><img src="img/ico/dolgon.png" width="12" height="12" alt="d"/><?php }
	    if ($gruppa_us == 'naemniki') {?><img src="img/ico/odinochkion.png" width="12" height="12" alt="n"/><?php }
	    if ($gruppa_us == 'mon') {?><img src="img/ico/monolit.png" width="12" height="12" alt="n"/><?php }?>
	  ...
	  if ($row['speed_w'] != 0) {?>
	  <p><img src="img/ico/avtomat.png" alt="a" width="12" height="12"/> <a href="attack.php?rand=0&weapon=avtomat&attack=<?php echo"$attack"; ?>">Стрелять с автомата</a> <?php
		if ($gruppa_us == 'svoboda') {?><img src="img/ico/svobodaon.png" width="12" height="12" alt="s"/><?php }
	    if ($gruppa_us == 'dolg') {?><img src="img/ico/dolgon.png" width="12" height="12" alt="d"/><?php }
	    if ($gruppa_us == 'naemniki') {?><img src="img/ico/odinochkion.png" width="12" height="12" alt="n"/><?php }
	    if ($gruppa_us == 'mon') {?><img src="img/ico/monolit.png" width="12" height="12" alt="n"/><?php }?>
...
$query_num = "Select id from users where gruppa='mon' and location='$location' and hp != '0' " ;
$result_num = mysqli_query($dbc, $query_num) or die ('Ошибка передачи запроса к БД11');
$number_mon = mysqli_num_rows($result_num); 
  ...
  <?php
    ...
    if ($number_mon != 0 and $row['gruppa'] != 'mon') {
  ?>
  <p><img src="img/ico/to4nost.png" width="12" height="12"/> <a href="attack.php?group=mon&rand=1">Напасть на "монолит"</a></p>
  <?php
  }
  ...
  ?> 
    <p><img src="img/ico/apte4ka.png" width="12" height="12"/> <a href="apt.php">Использовать аптечку 
	<?php 
	if ($apt_time <=60) { 
	$ttt = (60-$apt_time);
	echo "($ttt сек)"; }
