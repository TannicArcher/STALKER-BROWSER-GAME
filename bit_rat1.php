...
$clan = $row['clan'];
$query1 = "Select * from users where clan='$clan' order by bit_l desc limit 1";
...
<div id="main">
<div class="stats">
<center><p class="podmenu" style="border-top:1px solid #444e4f; background-color:#1c252f;">Потери здоровья в битве:</p></center>
</div>
	<?php
if (!empty($_GET['page'])) {
  $cur_page = $_GET['page'];
  ...
  $cur_page = 1;
	...
	if ($num_page > 0) {
$query_sub = "Select * from users where clan='$clan' order by bit_l desc limit $skip, $result_per_page";
...
?> <a <?php if ($id != $user_id) {?>class="white"<?php } else {?>class="red"<?php }?> href="profile.php?id=<?php echo "$id";?>"><?php echo "$nick";?></a> <small><span class="lal">[<?php echo "$bit_l";?>]</span></small></p>
