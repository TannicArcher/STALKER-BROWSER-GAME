...
<?php } ?>
  ...
  <p><img src="img/ico/point.png" width="12" height="12"/> <?php if ($type == 1) {?><span class="white">Сообщение</span><?php } else {?><a href="send1.php?type=1&set_id=<?php echo "$set_id";?>">Сообщение</a><?php }?></p>
  <p><img src="img/ico/point.png" width="12" height="12"/> <?php if ($type == 4) {?><span class="white">Хабар</span><?php } else {?><a href="send1.php?type=4&set_id=<?php echo "$set_id";?>">Хабар</a><?php }?></p>
  <p><img src="img/ico/point.png" width="12" height="12"/> <?php if ($type == 5) {?><span class="white">Аптечки</span><?php } else {?><a href="send1.php?type=5&set_id=<?php echo "$set_id";?>">Аптечки</a><?php }?></p>
  </div>
  <?php
////////////////////////////////////// Текст начало
if ($type == 1) {
  ...
  <div class="stats">
  <p>Сообщение:</p>
  <form enctype="multipart/form-data" method="post" action="mes.php?id=<?php echo "$set_id"; ?>">
  <textarea rows="2" cols="35px" name="say"></textarea>
  <div class="knopka">
...
if ($type == 9202) {
  ...
  if (!empty($thing)) {
	...
	if (!empty($row_isset_cl)) {
	  if ($row_cl['user_id'] <> $user_id) {
	   ?>
       <script type="text/javascript">
       document.location.href = "send1.php?type=2&set_id=<?php echo"$set_id";?>";
       </script>
       <?php
       exit(); 
	  }
		...
		if ($tip == 5) {
	      $query_inf = "Select name from ava where ava_id = '$inf_id' limit 1";
	      $result_inf = mysqli_query($dbc, $query_inf) or die ('Ошибка передачи запроса к БД');
	      $row_inf = mysqli_fetch_array($result_inf);
	    }//Название шлема
		if ($tip == 6) {
	      $query_inf = "Select name from art where art_id = '$inf_id' limit 1";
	      $result_inf = mysqli_query($dbc, $query_inf) or die ('Ошибка передачи запроса к БД');
	      $row_inf = mysqli_fetch_array($result_inf);
	    }//Название шлема
		if ($tip == 7) {
	      $query_inf = "Select name from detectors where dec_id = '$inf_id' limit 1";
	      $result_inf = mysqli_query($dbc, $query_inf) or die ('Ошибка передачи запроса к БД');
	      $row_inf = mysqli_fetch_array($result_inf);
	    }//Название шлема
		?>
	<p>
		...
		echo $row_inf['name'];?> [<a href="send1.php?type=2&set_id=<?php echo"$set_id";?>">отменить</a>]</p>
		<?php
	  }
	}
	else {
  ...
  if (isset($thing)) {?>
  <div class="stats">
  <p>[<a href="sendmes.php?type=2&set_id=<?php echo "$set_id";?>&thing=<?php echo "$thing";?>">ОТПРАВИТЬ</a>] Стоимость:<img src="img/ico/money.png" width="12" height="12"/><span class="white"> 1</span></p>
  </div>
  <?php
  }
  ?>
  ...
  while ($row_cl=mysqli_fetch_array($result_cl)) {
	...
	if ($tip == '4') {?><img src="img/ico/helmet.png" width="12" height="12" alt="h"/><?php }
    echo $row_inf['name'];?> [<a href="send1.php?type=2&set_id=<?php echo"$set_id";?>&thing=<?php echo $row_cl['thing_id']; ?>">выбрать</a>]</p>
	<?php
  }
  ?>
  </div>
...
//////////////////////////////////////Деньги начало
  if ($type == 9301) {
	...
	<div class="stats">
	<p>Деньги:</p>
    <form enctype="multipart/form-data" method="post" action="sendmes.php?type=3&set_id=<?php echo "$set_id"; ?>">
...
if ($type == 4) {
	...
	<div class="stats">
	<p>Хабар:</p>
    <form enctype="multipart/form-data" method="post" action="sendmes.php?type=4&set_id=<?php echo "$set_id"; ?>">
...
if ($type == 5) {
	...
	<div class="stats">
	<p>Аптечки:</p>
    <form enctype="multipart/form-data" method="post" action="sendmes.php?type=5&set_id=<?php echo "$set_id"; ?>">
...
//////////////////////////////////////Вмр начало
  if ($type == 13) {
	...
	<div class="stats">
	<p>WMR:</p>
    <form enctype="multipart/form-data" method="post" action="sendmes.php?type=13&set_id=<?php echo "$set_id"; ?>">
    <input type="text" class="input" name="money" style="width:50px;"/>
    <input type="submit" style="width:70px;" class="input" value="Отправить" name="send"/>
    </form>
