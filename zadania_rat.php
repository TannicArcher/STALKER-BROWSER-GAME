...
?>
    <div id="main">
    <div class="stats">
    <p class="podmenu">Рейтинг сталкеров по выполненным заданиям</p>
	</div>
   <?php
	$total = '100'; 
	?>
...
if (!empty($_GET['page'])) {
  ...
  $cur_page = 1;
	...
	if ($num_page > 0) {
	  $query_us = "Select quests, nick ,id,gruppa, last_active, lvl, m_kill, avatar from users where gruppa <> 'mytants' and gruppa <>'bandits' and gruppa <> 'zombie' and gruppa <> 'monolits' and m_kill <> '0' order by quests DESC limit $skip, $result_per_page";
	...
	$m_kill = $row_us['m_kill'];
	$quests = $row_us['quests'];
	    ?>
...
<div style="background:#2e2e2d url(http://stalkeronlinegame.epizy.com/link.png) repeat-x top;"><span class="white"><b>Задания: <img src="img/ico/yes.png"/><?php echo "$quests";?></b></span></div>
<img src="style/img/line.png" width="100%" height="2"/>
</div>
