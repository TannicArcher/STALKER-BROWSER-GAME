...
if ($ajax_mail != '1') {?>
  <p>Сообщение:</p><br/>
<form enctype="multipart/form-data" method="post" action="mes.php?id=<?php echo "$id2";?>" name="message">
<?php
...
<div class="knopka">
<input type="submit" class="input" value="Отправить" name="addad" accesskey="ы"/>
</div>
</form>
<?php } else {?>
<form action="sendMessage3.php?id=<?php echo "$id2";?>" method="post" name="message">
         <p>Сообщение:</p><br>
<?php
require_once('inc_smiles_panel.php');
?>
       <textarea name="msg" style="width:98%; height:60px;" rows="5" cols="50" id="message" autofocus required></textarea>
       </p>
...
<center>
<a href="mail4.php?id=<?php echo "$drug_id";?>" class="menu2">
...
?>
<?php
require_once('conf/navigaa.php');
?>
</div>

<?php
$query = "update message set reading='1' where ot='$drug_id' and dlya='$user_id' and reading='0' limit $total6 ";
$result = mysqli_query($dbc, $query) or die ('Ошибка передачи запроса к БД');
?>
<?php }?>
...
<?php if ($ajax_mail == '1') {?>
<small><p>Не работает почта?</p>
<A href="set_type.php?ajax_mail=on">Перейти в обычный режим почты</A></small>
<?php } else {?>
<small><A href="set_type.php?ajax_mail=off">Перейти в новый режим почты</A></small>
<?php }?>
</div>
...
<p class="podmenu" style="border-top:1px solid #444e4f;"></p>
<?php }?>
<p><img src="img/reload.gif" width="12" height="12" /> <a href="mail2.php">Назад</a> | <img src="img/ico/letter.png" width="12" height="12"/> <a href="send1.php?set_id=<?php echo "$drug_id"; ?>">Отправить посылку</a> | <a <?php if ($row_list != 0) {?>class="red"<?php } else {?><?php }?> href="black_list.php?id=<?php echo "$id2";?>"><?php if ($row_list != 0) {?>Удалить из Ч.С.<?php } else {?>Внести в Ч.С.<?php }?></a></p>
<p class="podmenu" style="border-top:1px solid #444e4f;"></p>
<?php
