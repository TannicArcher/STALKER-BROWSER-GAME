...
?>
    <div id="main">
    <div class="stats">
    <p class="podmenu">Рейтинг сталкеров по активности</p>
	</div>
	<?php
	$type=$_GET['type'];
	if (empty($type)) {
...
if (!empty($_GET['page'])) {
  ...
  $cur_page = 1;
	...
	if ($num_page > 0) {
	  $query_us = "Select nick ,id,gruppa, last_active, activ, lvl, avatar from users where gruppa <> 'mytants' and gruppa <>'bandits' and gruppa <> 'zombie' and gruppa <> 'monolits' order by activ DESC limit $skip, $result_per_page";
...
<div style="background:#2e2e2d url(http://stalkeronlinegame.epizy.com/link.png) repeat-x top;"><span class="white"><b>Активность: <img src="img/ico/point.png"/><?php echo $row_us['activ'];?></b></span></div>
<img src="style/img/line.png" width="100%" height="2"/>
</div>
</li>
