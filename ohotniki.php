...
?>
    <div id="main">
    <div class="stats">
    <p class="podmenu">Рейтинг сталкеров по убитым мутантам</p>
	</div>
   <?php
	$total = '100'; 
	?>
...
if (!empty($_GET['page'])) {
  ...
  $cur_page = 1;
	...
	if ($num_page > 0) {
	  $query_us = "Select nick ,id,gruppa, last_active, lvl, m_kill, avatar from users where gruppa <> 'mytants' and gruppa <>'bandits' and gruppa <> 'zombie' and gruppa <> 'monolits' and m_kill <> '0' order by m_kill DESC limit $skip, $result_per_page";
...
<div style="background:#2e2e2d url(http://sta1kers.ru/link.png) repeat-x top;"><span class="white"><b>Убито: <img src="img/ico/ohotniki.gif"/><?php echo "$m_kill";?> мутантов</b></span></div>
<img src="style/img/line.png" width="100%" height="2"/>
</div>
</li>
