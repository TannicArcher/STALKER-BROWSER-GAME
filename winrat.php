...
?>
    <div class="stats">
    <p class="podmenu">Рейтинг сталкеров по победам на арене</p>
	</div>
   <?php
	$total = '100'; 
	?>
...
if (!empty($_GET['page'])) {
  ...
  $cur_page = 1;
	...
	if ($num_page > 0) {
	  $query_us = "Select nick ,id,gruppa, last_active, lvl, win, avatar from users where gruppa <> 'mytants' and gruppa <>'bandits' and gruppa <> 'zombie' and gruppa <> 'monolits' and admin <> '1' order by win DESC, opit DESC limit $skip, $result_per_page";
...
<div style="background:#2e2e2d url(http://stalkeronlinegame.epizy.com/link.png) repeat-x top;"><span class="white"><b>Победы: <img src="img/ico/letterin1.png"/><?php echo "$win";?></b></span></div>
<img src="style/img/line.png" width="100%" height="2"/>
</div>
