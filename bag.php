...
<div id="main">
<div class="stats">
<p class="podmenu">Ваш рюкзак</p>
</div>
<?php
$user_id = $_SESSION['id'];
$query = "Select thing_id, type, inf_id, stat1, stat2, speed, sost, privat, need_lvl from things where user_id='$user_id' and place=0 limit 20";
$result = mysqli_query($dbc, $query) or die ('Ошибка передачи запроса к БД');
$total = mysqli_num_rows($result);
?>
<div class="stats">
<p class="white"> Вещей в рюкзаке [<?php echo "$total"; ?>/20]
...
<p><img src="img/ico/point.png" width="12" height="12"/> <a href="alldel.php" onclick="return confirm ('Уверены?')">Продать всё</a></p>
</div>
<?php
$err = $_GET['err'];
if (!empty($err)) {
  ...
  ?>
  <div id="error">
  Нельзя менять снаряжение во время битвы отрядов.
  </div>
  <?php
  }
}
...
?>
<div class="stats">
<p>У вас ничего нет в рюкзаке</p>
</div>
<?php
}
else {
...
?><?php
	...
	<div class="slot">
	  ...
	  <p>[<a href="instash.php?thing=<?php echo "$thing_id"?>">В тайник</a>] [<a href="wear.php?thing=<?php echo "$thing_id"?>">Установить</a>]  [<a href="out_thing.php?thing=<?php echo "$thing_id"?>" onclick="return confirm
('Уверены?')">
<?
...
?><?php
	...
	if ($type == 6) {////////Арты!!!
	$query_inf = "Select name, screen,klass from art where art_id='$inf_id' limit 1";
    $result_inf = mysqli_query($dbc, $query_inf) or die ('Ошибка передачи запроса к БД');
	$row_inf = mysqli_fetch_array($result_inf);
	$screen = $row_inf['screen'];
	$name = $row_inf['name'];
	...
	<div class="slot">
      ...
      <tbody>
      <tr>
      <td width="54" valign="top"><img src="img/art/<?php echo "$screen";?>" alt="<?php echo "$name";?>"/></td>
      <td width="106" valign="top">
	  <div class="clothes">
	    <span class="bonus"><?php echo "$name";?></span>
	 </div>
	  ...
	  <p>[<a href="wear.php?thing=<?php echo "$thing_id"?>">Повесить на пояс</a>]  [<a href="out_thing.php?thing=<?php echo "$thing_id"?>" onclick="return confirm
('Уверены?')">
<?
...
?><?php
	...
	<div class="slot">
	  ...
	  <p>[<a href="wear.php?thing=<?php echo "$thing_id"?>">Взять</a>] [<a href="instash.php?thing=<?php echo "$thing_id"?>">В тайник</a>]  [<a href="out_thing.php?thing=<?php echo "$thing_id"?>" onclick="return confirm
('Уверены?')">
<?
