...
$err=$_GET['err'];
if ($row['privat']<2 and $user_set == $user_id and !empty($_GET['lot']) and $row['place']<>3 and $row['place']<>2) {
if (!empty($_POST['send'])) {

if ($row_inf['price'] < 0 and $row_inf['klass']==1) {
  $price1 = 2000;
}
else {
  $price1 = $row_inf['price'];
}
if ($row_inf['price'] < 0 and $row_inf['klass']==2) {
  $price1 = 2000;
}
else {
  $price1 = $row_inf['price'];
}
if ($row_inf['price'] < 0 and $row_inf['klass']==3) {
  $price1 = 2000;
}
else {
  $price1 = $row_inf['price'];
}
if ($row_inf['price'] < 0 and $row_inf['klass']==4) {
  $price1 = 3500;
}
else {
  $price1 = $row_inf['price'];
...
$price_l1 = $row_inf['price'];
$price_l = ($price_l1 / '5');
$price_l = round("$price_l");
if ($price_l < '20') {
$price_l = '20';
}
$max_price = ($price_l * '2');
  ...
  $price_now = mysqli_real_escape_string($dbc, trim($price_now));
  $price= $_POST['price'];
  $price = mysqli_real_escape_string($dbc, trim($price));	
  $sgn = '#[1-9]#';
  if ($price_now>=$price) { $err=5;}
  if ($price_now<$price_l) { $err = 6; }
  if ($price<($price_l + 1)) {$err = 7; }
  if ($price_l > $price_now) { $err=11;}
  if ($max_price < $price_now or $max_price < $price) { $err=12;}
  if (!preg_match($sgn, $price)) { $err=8; }
  if (!preg_match($sgn, $price_now)) { $err=8; }
  $user_id=$_SESSION['id'];
  $query_isset = "Select money from users where id='$user_id' limit 1";
  $result_isset = mysqli_query($dbc, $query_isset) or die ('Ошибка передачи запроса к БД');
  $row_isset = mysqli_fetch_array($result_isset);
  if ($row_isset['money'] < 10) {$err=9; }
  if ($err == 0) {
    $query_isset = "update users set money=money-10 where id='$user_id' limit 1";
    $result_isset = mysqli_query($dbc, $query_isset) or die ('Ошибка передачи запроса к БД');
	$query_isset = "update things set place=3 where thing_id='$thing' limit 1";
    $result_isset = mysqli_query($dbc, $query_isset) or die ('Ошибка передачи запроса к БД');
	$query_isset = "insert into auction (`thing_id`, `type`, `date`, `price`, `price_now`, `price_user`) values ('$thing', '$type', NOW(), '$price', '$price_now', '0')";
    $result_isset = mysqli_query($dbc, $query_isset) or die ('Ошибка передачи запроса к БД');
    ?>
	<div class="stats">
    <p><img src="img/ico/shield.png" width="12" height="12"/> <a href="clothes.php?id=<?php echo "$user_id";?>">Моё снаряжение</a></p>
    </div>
	<div class="stats"><p class="bonus">Эта вещь успешно выставлена на аукцион</p></div><?php
...
if ($err == 11) { echo 'Ставка на данный лот должна быть не менее' . " $price_l RUB"; }
if ($err == 12) { echo 'Ставка на данный лот должна быть не более' . " $max_price RUB"; }
?>
</div>
<?php
}
...
/////////////////////Если это моя шмотка
  ...
  ?><div class="zx"><?php
	 if ($row['sost'] == 0) {?><p><a href="fix.php?thing=<?php echo "$thing";?>">Починить за <img src="img/ico/materials.png" width="12" height="12"/>1000</a></p>
	  ...
	  ?>
	  <p><img src="img/ico/upgrade.png" width="12" height="12"/><a href="upgrade.php?thing=<?php echo "$thing";?>">Улучшить у мастера</a></p>
	  <?php if ($row['privat'] < 2) {?>
	  <p><img src="img/ico/point.png" width="12" height="12"/><a href="thing.php?thing=<?php echo "$thing";?>&lot=1">Выставить на аукцион</a></p>
	  <?php }?>
	  </div>
	  </div>
	<?php
  } else {
  ?><p><img src="img/ico/upgrade.png" width="12" height="12"/><a href="upgrade.php?thing=<?php echo "$thing";?>">Список улучшений</a></p><?php
}
////////////////////////
}
//////////////////////////////////////Конец БРОНИ
...
if ($row['privat']<2 and $user_set == $user_id and !empty($_GET['lot']) and $row['place'] <> 2 and $row['place'] <> 3) {
?><div class="stats">
<p class="bonus"><b>Выставить лот на аукцион</b></p>
<form enctype="multipart/form-data" method="post" action="<? echo "$phpself"; ?>?thing=<?php echo "$thing"?>&lot=1">
<?php 
$price = $row_inf['price'];
$price = ($price / '5');
$price = round("$price");
...
$max_price = ($price * '2');
$price_buy = ($price + '500');
if ($price_buy > $max_price) {
$price_buy = $max_price;
}
?>
<p class="white">Начальная ставка: <img src="img/ico/money.png" width="12" height="12"/> <input type="text" class="input" name="price_now" value="<?php echo "$price"; ?>" style="width:50px;"/></p>
<p class="white">Выкуп: <img src="img/ico/money.png" width="12" height="12"/> <input type="text" class="input" name="price" value="<?php echo "$price_buy"; ?>" style="width:50px;"/></p><br />
<input type="submit" style="width:70px;" class="input" value="Выставить" name="send"/>
</form>
[Стоимость: <img src="img/ico/money.png" width="12" height="12"/>10 RUB]
</div>
<?php
}
///////////////////////////////
