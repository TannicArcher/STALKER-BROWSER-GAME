...
$at0 = $_GET['at'];
$br0 = $_GET['br'];
$at0 = mysqli_real_escape_string($dbc, trim($at0));
$br0 = mysqli_real_escape_string($dbc, trim($br0));
$at0 = round("$at0");
$br0 = round("$br0");
$yron = mysqli_real_escape_string($dbc, trim($yron));
$yron = round("$yron");
...
<div class="r2">
<center>
<?php if ($gerb1 > '0') {?><img src="img/gerb/<?php echo "$gerb1"; ?>.jpg" style="width: 60%; height: 110px;"/><?php } else {?><img src="img/gerb/<?php
if ($gruppa1 == 'naemniki') {?>o<?php }
if ($gruppa1 == 'svoboda') {?>s<?php }
if ($gruppa1 == 'dolg') {?>d<?php }
if ($gruppa1 == 'mon') {?>m<?php }
?>.jpg" style="width: 60%; height: 110px;"/><?php }?></center>
</div>
</td>
<td style="width: 50%">
<div class="r2">
<center>
<?php if ($gerb2 > '0') {?><img src="img/gerb/<?php echo "$gerb2"; ?>.jpg" style="width: 60%; height: 110px;"/><?php } else {?><img src="img/gerb/<?php
if ($gruppa2 == 'naemniki') {?>o<?php }
if ($gruppa2 == 'svoboda') {?>s<?php }
if ($gruppa2 == 'dolg') {?>d<?php }
if ($gruppa2 == 'mon') {?>m<?php }
?>.jpg" style="width: 60%; height: 110px;"/><?php }?></center>
</div>
</td>
</tr>
...
<div class="r6"><center>
<p class="net">Отряд <a class="stalker_link" href="company.php?company_id=<?php echo "$clan1";?>"><?php echo "$clan1_n";?></a> напал на отряд <a class="stalker_link" href="company.php?company_id=<?php echo "$clan2";?>"><?php echo "$clan2_n";?></a>.<br/> Битва начнётся через <?php
$timeb = $time_b;
...
?>.<br/> Есть время прихватить лишнюю обойму патронов.</p></center>
</div>
</div>
</div>
<?php } else {?>
...
<?php
$sostt2 = ($sosta2 * '2');
$boncl_h = ($hab_clan1 / $sostt2);
$boncl_h = round("$boncl_h");
$boncl_m = ($mon_clan1 / $sostt2);
$boncl_m = round("$boncl_m");
$bon10_h = ($hab_clan1 * '0.05');
$bon10_h = round("$bon10_h");
$bon10_m = ($mon_clan1 * '0.05');
$bon10_m = round("$bon10_m");
$msusd = ($minus_slava1 * '1.5');
$msusd = round("$msusd");
$boncl_m = '0';
$bon10_m = '0';
...
<?php
$sostt1 = ($sosta1 * '2');
$boncl_h = ($hab_clan2 / $sostt1);
$boncl_h = round("$boncl_h");
$boncl_m = ($mon_clan2 / $sostt1);
$boncl_m = round("$boncl_m");
$bon11_h = ($hab_clan2 * '0.05');
$bon11_h = round("$bon11_h");
$bon11_m = ($mon_clan2 * '0.05');
$bon11_m = round("$bon11_m");
$msusd = ($minus_slava2 * '1.5');
$msusd = round("$msusd");
$boncl_m = '0';
$bon11_m = '0';
...
<p><img src="img/ico/link.png"/> <a href="bitva.php?err=3">Рассчитать награды</a></p><?php }?>
<?php if ($clan_r > '7' and $gauss_at != '1' and $gt < $now) {?><p><img src="img/ico/link.png"/> <a class="bonus" href="gauss_on.php">Гаусс пушки заряжены и готовы к бою!</a></p><?php }?>
...
<center><a href="bitva.php"><img src="img/icon-refresh.png"/></a></center>
<p style="border-top: dashed 1px #444e4f"></p>
<?php if ($clan == $clan1) {?>
<?php
$query_sub = "Select * from users where clan = '$clan2' and hp>'0' order by hp desc limit 100";
$result_sub = mysqli_query($dbc, $query_sub) or die ('Ошибка передачи запроса к БД');
while ($row_sub = mysqli_fetch_array($result_sub)) {
$id_top = $row_sub['id'];
$name = $row_sub['nick'];
...
?>.</span><?php } else {?><a class="blue" href="bit_dop.php?id=<?php echo "$id_top" ; ?>&tip=1&cl=2" onclick="return confirm ('Уверены? Из склада вашего отряда заберут 1000 RUB.')">Пси-удар</a><?php }?><?php }?><br/>
<?php if ($time_pr > $now) {?><span class="red">Пистолет перезаряжается</span><?php } else {?><a class="menu" href="bit_attack.php?id=<?php echo "$id_top" ; ?>&tip=1&cl=2">Стрелять с пистолета</a><?php }?></p>
<p><?php if ($time_wr > $now) {?><span class="red">Автомат перезаряжается</span><?php } else {?><a class="menu" href="bit_attack.php?id=<?php echo "$id_top" ; ?>&tip=2&cl=2">Стрелять с автомата</a><?php }?></p>
<p style="border-top: dashed 1px #444e4f"></p>
<?php 
}
?>
<?php } else {?>
<?php
$query_sub = "Select * from users where clan = '$clan1' and hp>'0' order by hp desc limit 100";
$result_sub = mysqli_query($dbc, $query_sub) or die ('Ошибка передачи запроса к БД');
while ($row_sub = mysqli_fetch_array($result_sub)) {
$id_top = $row_sub['id'];
$name = $row_sub['nick'];
...
if ($time11 > '1799' and $time11 < '1860') {?>30 минут<?php }
if ($time11 > '1859' and $time11 < '1920') {?>31 минуту<?php }
?>.</span><?php } else {?><a class="blue" href="bit_dop.php?id=<?php echo "$id_top" ; ?>&tip=1&cl=1" onclick="return confirm ('Уверены? Из склада вашего отряда заберут 1000 RUB.')">Пси-удар</a><?php }?><?php }?><br />
<?php if ($time_pr > $now) {?><span class="red">Пистолет перезаряжается</span><?php } else {?><a class="menu" href="bit_attack.php?id=<?php echo "$id_top" ; ?>&tip=1&cl=1">Стрелять с пистолета</a><?php }?></p>
<p><?php if ($time_wr > $now) {?><span class="red">Автомат перезаряжается</span><?php } else {?><a class="menu" href="bit_attack.php?id=<?php echo "$id_top" ; ?>&tip=2&cl=1">Стрелять с автомата</a><?php }?></p>
<p style="border-top: dashed 1px #444e4f"></p>
<?php 
}
?>
